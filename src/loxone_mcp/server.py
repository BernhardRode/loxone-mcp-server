...
async def server_lifespan():
    ...
    try:
        ...
        logger.error(f"Unexpected error during Loxone MCP Server initialization: {e}", exc_info=True)
...
async def get_or_create_session(
    ...
    if not all([host, username, password]):
        logger.error(f"Cannot create session for client {client_id}: missing credentials (host={bool(host)}, username={bool(username)}, password={bool(password)})")
        return None
    ...
    try:
        ...
        # Create new session
        logger.info(f"Creating new session for client {client_id} connecting to {host}:{port or 80}")
...
def get_session_or_fallback(client_id: str) -> tuple[Optional[ClientSession], Optional['DeviceManager']]:
    """
...
async def loxone_list_devices(
    ...
    """
    logger.info(f"Listing devices for client {client_id} connecting to {host}:{port} - Type filter: {device_type}, Room filter: {room}")
    
    # Get or create session with provided credentials
    session = await get_or_create_session(client_id, host, username, password, port)
    
    if not session:
        error_msg = f"Failed to create session for client {client_id} - check credentials and connectivity"
        logger.error(error_msg)
        return create_error_response(
            ERROR_INITIALIZATION_FAILED,
            error_msg,
            context={"operation": "list_devices", "client_id": client_id, "host": host, "port": port}
        )
    ...
    try:
        ...
        return create_error_response(
            ...
            context={"operation": "list_devices", "filters": {"device_type": device_type, "room": room}}
        )
...
async def loxone_get_device_state(
    ...
    """
    logger.info(f"Getting device state for client {client_id} connecting to {host}:{port}, UUID: {uuid}")
    ...
    if not session:
        ...
        return create_error_response(
            ERROR_INITIALIZATION_FAILED,
            error_msg,
            context={"operation": "get_device_state", "client_id": client_id, "host": host, "port": port}
        )
    ...
    if not uuid:
        ...
        return create_error_response(
            ERROR_INVALID_PARAMETER,
            error_msg,
            details={"parameter": "uuid", "value": uuid}
        )
    ...
    try:
        ...
        try:
            ...
            if not enhanced_state.get("success", True):
                ...
                logger.warning(f"Enhanced state retrieval failed for {uuid}, falling back to basic state")
                device_state = {
                    "uuid": device.uuid,
                    "name": device.name,
                    "type": device.type,
                ...
                logger.info(f"Successfully retrieved enhanced state for device '{device.name}' ({uuid}) - Type: {device.type}")
                logger.debug(f"Enhanced state parameters: {list(enhanced_state.get('state', {}).keys())}")
...
@mcp.tool()
async def loxone_send_command(client_id: str = "default", uuid: str = "", value: str = "") -> dict[str, Any]:
    """
    ...
    if connection_manager is None:
        ...
        return create_error_response(
            ERROR_INITIALIZATION_FAILED,
            error_msg,
            context={"operation": "send_command", "client_id": client_id, "uuid": uuid, "value": value}
        )
...
@mcp.tool()
async def loxone_set_switch(client_id: str = "default", uuid: str = "", state: bool = False) -> dict[str, Any]:
    """
    ...
    if connection_manager is None:
        ...
        return create_error_response(
            ERROR_INITIALIZATION_FAILED,
            error_msg,
            context={"operation": "set_switch", "client_id": client_id, "uuid": uuid, "state": state}
        )
...
async def loxone_set_dimmer(uuid: str, brightness: int) -> dict[str, Any]:
    ...
    if loxone_client is None or device_manager is None:
        error_msg = "Loxone client or device manager not initialized - server may not be fully started"
        logger.error(error_msg)
        return create_error_response(
            ERROR_INITIALIZATION_FAILED,
            error_msg,
    ...
    if not isinstance(brightness, int):
        ...
        return create_error_response(
            ERROR_INVALID_PARAMETER,
            error_msg,
            details={"parameter": "brightness", "type": type(brightness).__name__, "value": brightness}
        )
    ...
    try:
        ...
        if device.type not in valid_dimmer_types:
            ...
            logger.warning(f"Dimmer operation attempted on non-dimmer device: {device.name} ({device.type})")
            return create_error_response(
        ...
        logger.debug(f"Setting brightness {brightness}% on dimmer '{device.name}' ({device.type}) in {device.room}")
...
async def loxone_set_cover_position(uuid: str, position: int) -> dict[str, Any]:
    ...
    try:
        ...
        if device.type not in valid_cover_types:
            ...
            logger.warning(f"Cover operation attempted on non-cover device: {device.name} ({device.type})")
            return create_error_response(
                ERROR_DEVICE_TYPE_MISMATCH,
                error_msg,
                details={
        ...
        logger.debug(f"Setting position {position}% on cover '{device.name}' ({device.type}) in {device.room}")
...
async def loxone_set_temperature(uuid: str, temperature: float) -> dict[str, Any]:
    ...
    if not isinstance(temperature, (int, float)):
        ...
        return create_error_response(
            ERROR_INVALID_PARAMETER,
            error_msg,
            details={"parameter": "temperature", "type": type(temperature).__name__, "value": temperature}
        )
    ...
    if not -50 <= temperature <= 50:
        ...
        return create_error_response(
            ERROR_VALIDATION_FAILED,
            error_msg,
            details={"parameter": "temperature", "value": temperature, "valid_range": "-50 to 50°C"}
        )
    ...
    try:
        ...
        if device.type not in valid_climate_types:
            ...
            logger.warning(f"Temperature operation attempted on non-climate device: {device.name} ({device.type})")
            return create_error_response(
                ERROR_DEVICE_TYPE_MISMATCH,
                error_msg,
                details={
        ...
        logger.debug(f"Setting temperature {temperature}°C on climate control '{device.name}' ({device.type}) in {device.room}")
...
async def loxone_send_secured_command(uuid: str, value: str, code: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not is_secured:
            logger.warning(f"Device '{device.name}' is not marked as secured, but using secured command anyway")
        
        logger.debug(f"Sending secured command '{value}' to device '{device.name}' ({device.type}) in {device.room}")
        ...
        try:
            ...
            if success:
                logger.info(f"Secured command sent successfully to '{device.name}' ({uuid}): {value}")
                return create_success_response({
                    "device": uuid,
                    "command": value,
                    "device_name": device.name,
            ...
            logger.error(f"Communication error sending secured command to {uuid}: {e}", exc_info=True)
            ...
            if success:
                ...
                return create_success_response({
                    ...
                    "security_info": {
                        ...
                        "note": "Secured scenes may require PIN authentication" if is_secured else None
                    },
                    "scene_info": {
                        "room": scene.room,
                        "category": scene.category
                ...
                if is_secured:
                    suggestions.insert(0, "Scene is secured - use loxone_send_secured_command with PIN if required")
                ...
                return create_error_response(
                    ...
                    context={
                        ...
                        "security_note": "Secured scenes may require PIN authentication" if is_secured else None
                    }
                )
...
@mcp.tool()
async def loxone_set_light_color(client_id: str = "default", uuid: str = "", color: str = "") -> dict[str, Any]:
    """
    ...
    if connection_manager is None:
        return create_error_response(ERROR_INITIALIZATION_FAILED, "Connection manager not initialized")
    
    session = await get_or_create_session(client_id)
    if not session:
        return create_error_response(ERROR_INITIALIZATION_FAILED, f"Failed to get session for client {client_id}")
    
    if not uuid or not color:
        return create_error_response(ERROR_INVALID_PARAMETER, "UUID and color parameters are required")
...
@mcp.tool()
async def loxone_set_climate_mode(client_id: str = "default", uuid: str = "", mode: str = "") -> dict[str, Any]:
    """
    ...
    if not uuid or not mode:
        return create_error_response(ERROR_INVALID_PARAMETER, "UUID and mode parameters are required")
...
@mcp.tool()
async def loxone_set_fan_speed(client_id: str = "default", uuid: str = "", speed: int = 0) -> dict[str, Any]:
    """
...
@mcp.tool()
async def loxone_set_volume(client_id: str = "default", uuid: str = "", volume: int = 0) -> dict[str, Any]:
    """
...
@mcp.tool()
async def loxone_arm_alarm(client_id: str = "default", uuid: str = "", code: str = "") -> dict[str, Any]:
    """
    ...
    if not uuid or not code:
        return create_error_response(ERROR_INVALID_PARAMETER, "UUID and code parameters are required")
...
@mcp.tool()
async def loxone_disarm_alarm(client_id: str = "default", uuid: str = "", code: str = "") -> dict[str, Any]:
    """
...
@mcp.tool()
async def loxone_set_text_input(client_id: str = "default", uuid: str = "", text: str = "") -> dict[str, Any]:
    """
...
@mcp.tool()
async def loxone_set_number_input(client_id: str = "default", uuid: str = "", value: float = 0.0) -> dict[str, Any]:
    """
...
def loxone_list_rooms(client_id: str = "default") -> dict[str, Any]:
    ...
    if not session and not device_mgr:
        return create_error_response(ERROR_INITIALIZATION_FAILED, "Neither session nor global instances available")
...
@mcp.tool()
async def loxone_control_room(client_id: str = "default", room: str = "", action: str = "") -> dict[str, Any]:
    """
    ...
    if not room or not action:
        return create_error_response(ERROR_INVALID_PARAMETER, "Room and action parameters are required")
    ...
    try:
        ...
        if result["success"]:
            ...
            return create_error_response(
                ERROR_COMMAND_FAILED,
                result.get("error", "Room control failed"),
                details=result
            )
    except Exception as e:
...
def main():
    ...
    logger.info("Server does not require environment credentials - clients provide credentials via tool parameters")
