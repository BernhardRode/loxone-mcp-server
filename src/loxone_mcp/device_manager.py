...
class LoxoneDevice:
    ...
    def _detect_capabilities(self) -> Dict[str, Any]:
        ...
        if self.type == DEVICE_TYPE_UP_DOWN_ANALOG:
            capabilities.update({
                ...
                "supports_increment_decrement": True
            })
        ...
        if self.type in [DEVICE_TYPE_INFO_ONLY_ANALOG, DEVICE_TYPE_INFO_ONLY_DIGITAL]:
            capabilities.update({
                "read_only": True,
                "sensor_type": "analog" if self.type == DEVICE_TYPE_INFO_ONLY_ANALOG else "digital"
            })
    ...
    def _initialize_metadata(self) -> Dict[str, Any]:
        ...
        if self.type in [DEVICE_TYPE_DIMMER, DEVICE_TYPE_LIGHT_CONTROLLER_V2]:
            ...
            metadata["validation_rules"] = {
                "position": {"type": "float", "min": 0, "max": 100}
            }
            ...
            metadata["state_parameters"] = {
                "value": f"Slider value ({min_val}-{max_val})"
            }
            metadata["parameter_units"] = {
                "value": unit
            }
            ...
            metadata["state_parameters"] = {
                "text": f"Text input value (max {max_length} chars)"
            }
            metadata["parameter_ranges"] = {
                "text": [0, max_length]
            }
            metadata["validation_rules"] = {
                "text": {"type": "str", "max_length": max_length}
            }
            ...
            metadata["state_parameters"] = {
                "value": f"Sensor reading ({sensor_type})"
            }
            metadata["parameter_units"] = {
                "value": unit
            }
            ...
            metadata["state_parameters"] = {
                "active": "Scene is currently active"
            }
            metadata["parameter_ranges"] = {
                "active": [0, 1]
            }
...
class DeviceManager:
    ...
    def load_devices(self, loxapp_json: Dict[str, Any]) -> None:
        ...
        if unsupported_types:
            logger.info(f"Unsupported device types encountered: {', '.join(sorted(unsupported_types))}")
                    ...
                    try:
                        ...
                        logger.error(
                            ...
                            extra={
                                ...
                                "state_data_keys": list(validated_data.keys()) if isinstance(validated_data, dict) else 'non-dict'
                            }
                        )
            else:
                logger.debug(
    ...
    async def control_room(self, room: str, action: str) -> Dict[str, Any]:
        ...
        if action == "lights_off":
            target_devices = [
                ...
                if d.type in [DEVICE_TYPE_SWITCH, DEVICE_TYPE_DIMMER, 
                             DEVICE_TYPE_LIGHT_CONTROLLER_V2, DEVICE_TYPE_COLOR_PICKER_V2]
            ...
            target_devices = [
                d for d in room_devices 
                if d.capabilities.get("supports_on_off", False)
            ]
        ...
        logger.info(f"Room control '{action}' prepared for {len(affected_devices)} devices in room '{room}'")
        return result
    ...
    def get_enhanced_device_state(self, uuid: str) -> Dict[str, Any]:
        ...
        with self._state_lock:
            if not device.states or len(device.states) == 0:
                logger.warning(f"Empty state cache for device {uuid} ({device.name}), may need state update")
    ...
    def extract_device_capabilities(self, device_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if device_type in [DEVICE_TYPE_DIMMER, DEVICE_TYPE_LIGHT_CONTROLLER_V2, DEVICE_TYPE_COLOR_PICKER_V2]:
    ...
    def _build_color_light_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "units": "%",
            "format": "%.1f%%",
            "state_parameters": ["value", "on", "brightness", "brightness_pct", "color", "color_hex"],
    ...
    def _build_climate_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "units": "°C",
            "format": "%.1f°C",
            "state_parameters": ["temperature", "current_temperature", "target_temperature", "mode", "mode_str"],
    ...
    def _build_switch_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "state_parameters": ["value", "active", "on", "state"],
            "control_type": "switch"
        })
    ...
    def _build_media_player_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "units": "%",
            "format": "%.1f%%",
            "state_parameters": ["volume", "volume_pct", "active", "mode", "state", "is_playing", "is_paused"],
    ...
    def _build_scene_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "state_parameters": ["active", "is_active", "last_triggered"],
            "control_type": "scene"
        })
    ...
    def _build_generic_state(self, device: LoxoneDevice, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base["metadata"].update({
            "state_parameters": list(device.states.keys()),
